...
const CATEGORIES = ['scam', 'radicalization', 'money_laundering', 'digital_resilience', 'youth_peace'];
...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  radicalization:     'bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300',
  money_laundering:   'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300',
  digital_resilience: 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300',
  youth_peace:        'bg-accent-50 text-accent-700 dark:bg-accent-900/30 dark:text-accent-300',
};
...
export default function Insights() {
  ...
  return (
    <>
      <SectionHero title={t('insights.title')} subtitle={t('insights.subtitle')} />

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* ── Filters ────────────────────────────────────────────────────── */}
        <div className="flex flex-col sm:flex-row gap-4 mb-10">
          <label className="sr-only" htmlFor="search-input">{t('insights.search_placeholder')}</label>
          <input
            id="search-input"
            type="search"
            value={search}
          ...
          <label className="sr-only" htmlFor="category-select">{t('insights.filter_category')}</label>
          <select
            id="category-select"
            value={category}
          ...
          >
            ...
            {CATEGORIES.map((c) => (
              <option key={c} value={c}>{t(`insights.categories.${c}`, { defaultValue: c })}</option>
            ))}
          </select>

          <label className="sr-only" htmlFor="language-select">{t('insights.filter_language')}</label>
          <select
            id="language-select"
            value={language}
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-400 py-16">{t('insights.no_results')}</p>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
